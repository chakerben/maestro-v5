...
5. Commit through `maestro-vcs:00-commit` (gate + secrets scan), message
   `fix(<scope>): <cause in one clause>` with `Refs: maestro_docs/tasks/<folder>`.
...
- The commit went through `00-commit` (gate summary line in the report).
